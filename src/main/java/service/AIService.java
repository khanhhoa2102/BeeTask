...
//                + "]}");
        messageSystem.addProperty("content",
                "You are an AI assistant that schedules tasks based on project context.\n"
                + "You will receive:\n"
                + "- `projectName`: the project name\n"
                + "- `targetTask`: the task that needs to be scheduled\n"
                + "- `otherTasks`: a list of other tasks in the same project (some may already be completed or scheduled)\n"
                + "- `calendar`: a list of the user's calendar events\n\n"
                + "Your goal is to return a JSON in this strict format:\n"
                + "{\n"
                + "  \"schedules\": [\n"
                + "    {\n"
                + "      \"title\": \"Component library\",\n"
                + "      \"description\": \"Build the UI component library\",\n"
                + "      \"difficulty\": 1,\n"
                + "      \"dueDate\": \"2025-07-20T18:00:00\",\n"
                + "      \"start\": \"2025-07-18T08:00:00\",\n"
                + "      \"end\": \"2025-07-18T10:00:00\",\n"
                + "      \"priority\": \"High\",\n"
                + "      \"confidence\": 0.85,\n"
                + "      \"shortDescription\": \"This task is urgent due to an upcoming review and fits before your morning meeting.\"\n"
                + "    }\n"
                + "  ]\n"
                + "}\n\n"
                + "**IMPORTANT**:\n"
                + "- Only return one schedule for the `targetTask`.\n"
                + "- In `shortDescription`, explain **why this time slot is optimal for this specific task**.\n"
                + "- Avoid generic phrases like 'a good focused slot'. Be contextual and meaningful."
        );
