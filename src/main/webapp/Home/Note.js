...
class NoteManager {
    ...
    handleNoteActions(e) {
        ...
        if (!actionItem || !this.currentNoteId)
            return;
        ...
        switch (action) {
            case "setLabel":
                ...
                break;
            case "setDeadline":
                this.openModal("deadline");
                break;
            case "delete":
                this.openModal("delete");
                break;
            default:
                this.showNotification("❗ Tính năng chưa hỗ trợ", "error");
        }
    ...
    addNewNote() {
        ...
        if (!userId) {
            this.showNotification("❌ UserId not found", "error");
            return;
        }
        ...
        formData.append("title", "New notes");
        formData.append("content", "Description content...");
        ...
        fetch("notes", {
            ...
            body: formData
                ...
                .then(message => {
                    ...
                    console.log("Created note successfully:", message);
                    this.showNotification("✅ Notes created", "success");
    ...
    createNewLabel() {
        ...
        if (!name)
            return this.showNotification("⚠️Label name cannot be blank", "error");
    ...
    saveDeadline() {
        ...
        if (!date || !time || !this.currentNoteId)
            return;
    ...
    confirmDelete() {
        if (!this.noteToDelete || !this.currentNoteId)
            return;
        ...
        fetch("notes", {
            ...
            body: new URLSearchParams({action: "delete", noteId: this.currentNoteId})
                ...
                .then(res => res.ok ? res.text() : Promise.reject("❌ Cannot be deleted"))
                .then(() => {
                    this.noteToDelete.remove();
                    this.showNotification("🗑 ️Note deleted", "success");
                    this.checkEmptyState();
                })
                .catch(err => {
                    console.error(err);
